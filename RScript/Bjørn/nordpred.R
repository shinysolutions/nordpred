...
nordpred <- function(cases,pyr,startestage,startuseage,noperiods=NULL,recent=NULL,
  ...
  # Written by:	Bj?rn M?ller and Harald Fekjaer <hf@kreftregisteret.no>, 2000-2003
...
nordpred.estimate <- function(cases,pyr,noperiod,startestage,linkfunc="power5") {
  # Nordpred:   R (www.r-project.org) & S-PLUS (www.insightful.com) functions 
  #             for prediction of cancer incidence (as used in the Nordpred project).
  # Written by:	Bj?rn M?ller and Harald Fekjaer <hf@kreftregisteret.no>, 2000-2002
  # See also:	http://www.kreftregisteret.no/software/nordpred/
  # License:	GNU version 2 
  ...
  power5link$linkfun <- function(mu)  { (mu)^0.2 }
  power5link$linkinv <- function(eta) { eta^5 }
  power5link$mu.eta <- function(eta)  { 5*eta^4 }
  ...
  # Estimation:
  if (linkfunc=="power5") {
    res.glm <- glm(Cases~as.factor(Age)+Period+as.factor(Period)+as.factor(Cohort)+ offset((y)^.2) -1,family=power5link,data=apcdata)
